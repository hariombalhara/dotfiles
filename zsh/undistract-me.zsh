...
function notifyosd-precmd() {
    ...
    if [[ ${cmdignore[(r)$cmd_basename]} == $cmd_basename ]]; then
        ...
        if [ ! -z "$cmd" -a $cmd_time -gt 10 ]; then
            if [ ! -z $SSH_TTY ] ; then
                notify-send -i utilities-terminal -u low "$cmd_basename on `hostname` completed $cmdstat" "\"$cmd\" took $cmd_time seconds"; pacmd play-file $sndstat 1
            else
                notify-send -i utilities-terminal -u low "$cmd_basename completed $cmdstat" "\"$cmd\" took $cmd_time seconds"; pacmd play-file $sndstat 1
            fi
        fi
        unset cmd
    fi
